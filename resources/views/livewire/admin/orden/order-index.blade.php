<div>
                ...
                <tbody>
                    @foreach ($ventas as $venta)
                        <tr class="bg-white dark:bg-gray-800">
                            ...
                            <td class="px-6 py-4">{{ $venta->monto_total }}</td>
                            <td class="px-6 py-4">{{ $venta->orden ? $venta->orden->estado : 'Pendiente' }}</td>
                            <td class="px-6 py-4">
                                <a href="{{ route('admin.detalle_ventas.show', $venta) }}">Ver</a>
                            </td>
                            <td class="px-6 py-4">
                                @if ($venta->factura == null)
                                    <span>Móvil</span>
                                @else
                                    <a href="{{ $venta->factura }}" target="_blank">Factura</a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
    ...
    @else
        <div class="flex items-center p-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
            ...
            <div>
                <span class="font-medium">Info alert!</span> Aun no se registró una nota de venta.
            </div>
        </div>
    @endif
</div>
